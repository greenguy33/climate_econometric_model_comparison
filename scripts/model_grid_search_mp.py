...
class RegressionResult:
	...
	def save_model_to_file(self):
		with open(f"output/models/{self.target_name}_best_model_from_grid_search_mp.csv", "w") as write_file:
...
def run_fe_regression_with_cv(num_folds, target_name, target_var, weights, model_vars, fixed_effects, incremental_effects):
	...
	for fold in range(num_folds):
		train_data = pd.read_csv(f"data/regression/cross_validation/{target_name}_regression_data_insample_{str(fold)}.csv")
		test_data = pd.read_csv(f"data/regression/cross_validation/{target_name}_regression_data_outsample_{str(fold)}.csv")
...
def find_best_model(fe, ie, weights, num_folds=1):
...
for target_name, target_var in target_var_list.items():
	...
	pool = ThreadPool(6)
	models = pool.starmap(find_best_model, ((fe, ie, weights) for fe in effect_variations["fixed_effects"] for ie in effect_variations["incremental_effects"] for weights in effect_variations["weights"]))
	pool.close()
	pool.join()
