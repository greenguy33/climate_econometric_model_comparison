...
class RegressionResult:
	...
	def save_model_to_file(self):
		with open(f"output/models/{self.target_name}_best_model_from_grid_search_mp_mt.csv", "w") as write_file:
...
def run_fe_regression_with_cv(num_folds, target_name, target_var, weights, model_vars, fixed_effects, incremental_effects):
	...
	for fold in range(num_folds):
		...
		in_sample_pred_int_acc.append(calculate_prediction_interval_accuracy(copy.deepcopy(train_data_covariates), copy.deepcopy(train_data[target_var]), copy.deepcopy(in_sample_predictions), copy.deepcopy(cov_mat)))
		...
		out_sample_pred_int_acc.append(calculate_prediction_interval_accuracy(copy.deepcopy(test_data_covariates), copy.deepcopy(test_data[target_var]), copy.deepcopy(out_sample_predictions), copy.deepcopy(cov_mat)))
...
def find_best_model(fe, ie, weights, target_name, target_var, num_folds=10):
...
effect_variations = {
	"fixed_effects":[0,"year","country","country + year"],
	"incremental_effects":[0,1,2,3],
	"weights":["unweighted","ag_weighted","pop_weighted"]
}
...
for target_name, target_var in target_var_list.items():
	...
	pool = ThreadPool(10)
	models = pool.starmap(find_best_model, ((fe, ie, weights, target_name, target_var) for fe in effect_variations["fixed_effects"] for ie in effect_variations["incremental_effects"] for weights in effect_variations["weights"]))
	pool.close()
	pool.join()
