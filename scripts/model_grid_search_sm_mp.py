...
def calculate_prediction_interval_accuracy(y, predictions):
	...
	return np.mean(np.mean(pred_data.pred_int_acc))
...
def find_best_model(fe, ie, weights):
...
for target_name, target_var in target_var_list.items():
	...
	permutations = it.product(effect_variations["fixed_effects"],effect_variations["incremental_effects"],effect_variations["weights"])

	with ThreadPool() as pool:
		models = pool.starmap(find_best_model, permutations)
